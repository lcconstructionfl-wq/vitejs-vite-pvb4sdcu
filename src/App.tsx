...
type ManualStepObj = {
  ...
  customDiagram?: string | null;
};

type ManualStep = string | ManualStepObj;

type PreflightItemObj = {
  text: string;
  url?: string;
  tag?: string;
};

type PreflightItem = string | PreflightItemObj;
...
const AGENT_PERSONAS: Record<string, AgentPersona> = {
  plumbing: { 
    ...
    greeting: "I'm ready. Ask me about the manual, specific IPC codes, or general plumbing theory.",
    color: "bg-blue-600",
  ...
  electrical: { 
    ...
    greeting: "I'm online. I can help with this specific manual, load calcs, or general electrical theory.",
    color: "bg-yellow-600",
  ...
  carpentry: { 
    ...
    greeting: "Ready to build. Ask me about the prints, framing geometry, or general woodworking tips.",
    color: "bg-amber-600",
  ...
  tile: { 
    ...
    greeting: "Let's set some tile. Ask about the manual, thin-set types, or layout math.",
  ...
  default: {
    ...
    greeting: "I have access to all manuals and general construction knowledge. How can I help?",
    color: "bg-slate-800",
...
const BlueprintDiagram: React.FC<BlueprintDiagramProps> = ({ type, customUrl }) => {
  if (!type) return null;

  const renderContent = () => {
    switch (type) {
      case 'custom':
        if (!customUrl) return <div className="text-blue-300 text-xs text-center p-4">No custom blueprint URL provided.</div>;
        return (
            <img 
                src={customUrl} 
                ...
                onError={(e) => {
                    ...
                    target.onerror = null; 
                    target.src="https://via.placeholder.com/400x200?text=Blueprint+Load+Error"
                }}
            />
        );
      case 'layout':
        return (
          <svg viewBox="0 0 400 200" className="w-full h-full text-blue-100" stroke="currentColor" fill="none" strokeWidth="2">
            <path d="M 20 100 L 380 100" strokeDasharray="5,5" opacity="0.5" />
            <text x="200" y="30" fill="currentColor" fontSize="12" textAnchor="middle" stroke="none">SYSTEM FLOW ORDER</text>
            <rect x="50" y="60" width="40" height="80" rx="4" />
            <text x="70" y="155" fill="currentColor" fontSize="10" textAnchor="middle" stroke="none">Filter</text>
            <text x="70" y="90" fill="currentColor" fontSize="8" textAnchor="middle" stroke="none">5mic</text>
            <rect x="150" y="50" width="60" height="100" rx="8" />
            <text x="180" y="165" fill="currentColor" fontSize="10" textAnchor="middle" stroke="none">Iron (Air)</text>
            <rect x="270" y="50" width="60" height="100" rx="8" />
            <text x="300" y="165" fill="currentColor" fontSize="10" textAnchor="middle" stroke="none">Softener</text>
      ...
      case 'bypass':
        return (
          <svg viewBox="0 0 400 220" className="w-full h-full text-blue-100" stroke="currentColor" fill="none" strokeWidth="2">
             <text x="200" y="30" fill="currentColor" fontSize="12" textAnchor="middle" stroke="none">3-VALVE BYPASS SCHEMATIC</text>
            <path d="M 40 180 L 360 180" strokeWidth="3" />
            <text x="200" y="200" fill="currentColor" fontSize="10" textAnchor="middle" stroke="none">Bypass Valve (Normally CLOSED)</text>
            ...
            <rect x="130" y="40" width="140" height="60" rx="4" strokeDasharray="4,4" />
            <text x="200" y="75" fill="currentColor" fontSize="14" textAnchor="middle" stroke="none">TREATMENT UNIT</text>
          </svg>
        );
      case 'airgap':
        return (
          <svg viewBox="0 0 200 200" className="w-full h-full text-blue-100" stroke="currentColor" fill="none" strokeWidth="2">
            <text x="100" y="20" fill="currentColor" fontSize="12" textAnchor="middle" stroke="none">AIR GAP DETAIL</text>
            <path d="M 100 40 L 100 120" strokeWidth="6" />
            <line x1="80" y1="120" x2="120" y2="120" />
            <path d="M 130 120 L 130 160" stroke="#FFC947" markerStart="url(#tick)" markerEnd="url(#tick)" />
            <text x="145" y="145" fill="#FFC947" fontSize="10" stroke="none">2" MIN</text>
            <ellipse cx="100" cy="170" rx="30" ry="10" fill="rgba(255,255,255,0.1)" />
            <path d="M 70 170 L 70 190 L 130 190 L 130 170" />
            <text x="100" y="195" fill="currentColor" fontSize="10" textAnchor="middle" stroke="none">Floor Drain</text>
          </svg>
        );
      default:
        return null;
  ...
  return (
    <div className="w-full bg-[#0F2640] rounded-lg overflow-hidden border border-slate-600 shadow-inner mt-4 mb-2">
      <div className="bg-[#08121f] px-3 py-1 flex justify-between items-center border-b border-slate-700">
        <span className="text-[10px] font-mono text-blue-300 tracking-widest uppercase">LC-STD-DWG-{type.toUpperCase()}</span>
    ...
    manuals: [
      {
        ...
        description: "Critical measurements and pipe securing protocols for rough-ins.",
        lastUpdated: "Oct 12, 2023",
        content: {
          overview: "All rough-in plumbing must strictly adhere to local code and LC Custom internal tolerances to ensure seamless finish work.",
          steps: [
          ...
          critical: "Do not bury any PEX fittings behind drywall without photographic evidence and site supervisor approval."
        }
      },
      {
        id: "p-02",
        title: "PVC Glue & Primer Procedure",
        description: "Correct application of purple primer and cement to prevent leaks.",
        lastUpdated: "Sep 05, 2023",
        content: {
          overview: "Failures in PVC joints are almost always due to improper priming or setting time.",
          steps: [
          ...
          critical: "Any joint lacking visible purple primer will be rejected immediately."
        }
      },
      {
        id: "p-03",
        title: "Well Water Install (Option A)",
        description: "Complete installation guide for Well Water systems including pre-flight checks.",
        lastUpdated: "Dec 11, 2025",
        content: {
          overview: "Installation protocol for standard Option A well water treatment. Follow component order strictly: Pressure Tank -> Sediment -> Iron (Air) -> Softener.",
          resources: [
          ...
          steps: [
            ...
            { 
              ...
              diagram: "layout",
              exampleImage: "https://images.unsplash.com/photo-1581094794329-c8112a89af12?q=80&w=2732&auto=format&fit=crop"
            },
            { 
          ...
          critical: "Bonding Jumper Wire and Air Gap Fittings are CODE REQUIREMENTS. Failure to install these will result in immediate inspection failure and non-payment."
        }
      }
    ]
  },
    ...
    manuals: [
      {
        ...
        content: {
          overview: "Waterproofing the foundation is the first line of defense against basement moisture. This process must be continuous and defect-free.",
          steps: [
          ...
          critical: "Do not backfill until the membrane has fully cured (24 hours minimum) and has been inspected."
        }
      }
    ]
  },
    ...
    manuals: [
      {
        ...
        content: {
          overview: "A perfect tile job fails without a perfect substrate. We do not skip waterproofing steps.",
          steps: [
          ...
          critical: "Never use drywall screws for cement board. Use alkali-resistant screws only."
        }
      },
      {
        id: "t-02",
        ...
        content: {
          overview: "Haze left overnight requires acid washing, which risks damage. Clean as you go.",
          steps: [
          ...
          critical: "Do not dump grout water in clients' sinks or bathtubs. Use designated washout area."
        }
      }
    ]
  },
    ...
    manuals: [
      {
        ...
        content: {
          overview: "Wall ties are the structural integrity of the veneer. Spacing is non-negotiable.",
          steps: [
          ...
          critical: "Ensure weep holes are installed every 32 inches at the bottom course."
        }
      }
    ]
  },
    ...
    manuals: [
      {
        ...
        content: {
          overview: "LC Custom is known for seamless trim work. We cope inside corners; we do not miter them.",
          steps: [
          ...
          critical: "Do not leave hammer marks on trim. Use a nail set or gun adjustment."
        }
      },
      {
        id: "c-02",
        ...
        content: {
          overview: "Water intrusion is the #1 liability. Follow the flashing tape sequence strictly.",
          steps: [
          ...
          critical: "Never tape the bottom flange. Water must be able to escape if it gets behind."
        }
      }
    ]
  },
    ...
    manuals: [
      {
        ...
        content: {
          overview: "Do not treat Cat6 like Romex. Kinks and staples destroy signal quality.",
          steps: [
    ...
    manuals: [
      {
        ...
        content: {
          overview: "A clean site is a safe site. All trades are responsible for their own debris.",
          steps: [
          ...
          critical: "Food trash must be removed from the site daily to prevent pests."
        }
      }
    ]
  }
...
const ICON_MAP: Record<string, LucideIcon> = {
  Hammer, Droplet, BrickWall, Grid3X3, Zap, HardHat, FileText, Waves, Wrench, ShieldCheck
};

interface AIChatWidgetProps {
  categoryId: string;
  manualContext: string | null;
  manualData: ManualContent | null;
  externalQuery: string | null;
  onExternalQueryHandled: () => void;
}

const AIChatWidget: React.FC<AIChatWidgetProps> = ({ categoryId, manualContext, manualData, externalQuery, onExternalQueryHandled }) => {
  ...
  const callGemini = async (userMessage: string) => {
    ...
    const contextString = manualData ? JSON.stringify(manualData) : "No specific manual open.";
    ...
    try {
        ...
        return data.candidates?.[0]?.content?.parts?.[0]?.text || "I'm having trouble analyzing the data. Please check your connection.";
    } catch (error) {
  ...
  const handleSend = async (overrideInput: string | null = null) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
      {/* Chat Window */}
      <div 
        className={`pointer-events-auto bg-white rounded-2xl shadow-2xl border border-slate-200 w-[calc(100vw-48px)] md:w-96 overflow-hidden transition-all duration-300 origin-bottom-right mb-4 ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4 pointer-events-none h-0'}`}
      >
        {/* Header */}
        <div className={`${agent.color} p-4 flex justify-between items-center text-white shadow-md`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-sm leading-tight flex items-center gap-2">
                  {agent.name}
                  <span className="bg-white/20 text-[10px] px-1.5 py-0.5 rounded font-mono">Gemini</span>
              </h3>
              <p className="text-[10px] opacity-90 uppercase tracking-wider font-medium">{agent.role}</p>
            </div>
          </div>
        ...
        <div className="h-80 bg-slate-50 overflow-y-auto p-4 space-y-4">
          <div className="text-center">
              <span className="text-[10px] text-slate-400 font-medium uppercase tracking-widest bg-slate-100 px-2 py-1 rounded-full">Context: {manualContext || 'General'}</span>
          </div>
      ...
      >
        ...
        {!isOpen && (
            <div className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 flex flex-col items-start pl-0 group-hover:pl-2">
                <span className="whitespace-nowrap text-sm font-bold">Ask {agent.name.split(' ')[0]}</span>
                <span className="whitespace-nowrap text-[10px] opacity-90">Expert Mode Active</span>
            </div>
        )}
      </button>
    </div>
...
const Header: React.FC<HeaderProps> = ({ onHome, onSearch, searchTerm, onAdminClick, isAdmin }) => {
      ...
      <div className="max-w-4xl mx-auto px-4 py-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center space-x-2 cursor-pointer" onClick={onHome}>
            <div className="bg-[#FFC947] p-1.5 rounded text-[#0C1A2A] font-bold">LC</div>
            <div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-[#1A2C42] rounded-lg mb-3 overflow-hidden animate-fade-in">
            ...
            <button 
              ...
              className={`block w-full text-left px-4 py-3 text-sm ${isAdmin ? 'text-red-400 hover:bg-red-900/20' : 'text-[#FFC947] hover:bg-yellow-900/20'}`}
            >
...
const CategoryCard: React.FC<CategoryCardProps> = ({ categoryKey, data, onClick }) => {
  ...
  return (
    ...
    >
      <div className={`p-4 rounded-full mb-3 ${data.color} group-hover:scale-110 transition-transform`}>
        <Icon size={32} />
      </div>
      <h3 className="font-bold text-slate-800 text-lg">{data.title}</h3>
      <p className="text-slate-500 text-xs mt-1">{data.manuals.length} Protocols</p>
    </button>
  );
};
...
interface ManualListItemProps {
  ...
  progress?: { [stepId: string]: boolean };
}

const ManualListItem: React.FC<ManualListItemProps> = ({ manual, onClick, isAdmin, onDelete, onView, progress }) => {
      ...
      >
        ...
        <div className="flex-1 pr-4">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-semibold text-slate-800 group-hover:text-orange-600 transition-colors">{manual.title}</h4>
            {isAdmin && <Edit size={14} className="text-blue-500 opacity-50" />}
            {isComplete && !isAdmin && <CheckCircle2 size={16} className="text-emerald-500" />}
          </div>
          <p className="text-xs text-slate-500 line-clamp-1">{manual.description}</p>
          ...
          {!isAdmin && totalCount > 0 && (
              <p className="text-[10px] font-bold tracking-wider uppercase mt-2 text-slate-400">
                  {isComplete ? (
                      ...
                      <span className="text-orange-500">{completedCount}/{totalCount} Steps Completed</span>
                  ) : (
        ...
        <div className="flex items-center gap-2">
            {isAdmin ? (
                <>
                     <button 
                        onClick={(e) => { e.stopPropagation(); onView(manual); }}
...
interface ManualWizardProps {
  ...
  onBack: () => void;
  progress?: { [stepId: string]: boolean };
  onToggleCheck: (index: string | number) => void;
  onReset: () => void;
  onAskAI: (query: string) => void;
}

const ManualWizard: React.FC<ManualWizardProps> = ({ manual, onBack, progress, onToggleCheck, onReset, onAskAI }) => {
    // Determine screens: 
    // 0: Intro/Overview
    ...
    const hasPreflight = manual.content.preflight && manual.content.preflight.length > 0;
    ...
    const isStep = currentScreen >= stepOffset && currentScreen < (stepOffset + stepsCount);
    ...
    const checklistPercent = totalChecklist > 0 ? Math.round((completedCount / totalChecklist) * 100) : 0;
    ...
    const currentStepData = isStep ? manual.content.steps[currentStepIndex] : null;
    const stepText = isStep ? (typeof currentStepData === 'string' ? currentStepData : currentStepData?.text) : '';
    const stepDiagram = isStep ? (typeof currentStepData === 'object' ? currentStepData?.diagram : null) : null;
    const stepCustomDiagram = isStep ? (typeof currentStepData === 'object' ? currentStepData?.customDiagram : null) : null;
    const stepImage = isStep ? (typeof currentStepData === 'object' ? currentStepData?.image : null) : null;
    const stepExampleImage = isStep ? (typeof currentStepData === 'object' ? currentStepData?.exampleImage : null) : null;
                ...
                <div className="flex justify-between items-center mb-3">
                    <div className="flex flex-col">
                        ...
                        <span className="text-[#FFC947] text-xs font-bold tracking-wider uppercase opacity-90">
                            {isOverview ? "Overview" :
                             isPreflight ? "Pre-Flight Check" :
                             isStep ? `Action Step ${currentStepIndex + 1}` :
                             isCritical ? "Critical Warning" : "Final Inspection"}
                        </span>
                    </div>
            ...
            <div className="flex-grow flex flex-col relative bg-white rounded-b-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                {isOverview && (
                    <div className="p-8 flex flex-col h-full items-center text-center animate-fade-in overflow-y-auto">
                        ...
                        <h2 className="text-2xl md:text-3xl font-bold text-[#0C1A2A] mb-4">Start Guide</h2>
                        <p className="text-slate-600 text-lg leading-relaxed max-w-lg mb-8">{manual.content.overview}</p>
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg text-left w-full max-w-sm border border-slate-200 mb-6">
                         ...
                         {manual.content.resources && (
                            <div className="w-full max-w-sm mb-4">
                                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 text-left">Training & Docs</h4>
                                <div className="space-y-2 text-left">
                                {manual.content.resources.map((res, i) => (
                                    <a key={i} href={res.url} className="flex items-center p-3 bg-white rounded-lg border border-slate-200 text-slate-700 hover:border-[#FFC947] hover:shadow-sm transition-all group">
                                    {res.type === 'video' ? <Youtube size={18} className="text-red-500 mr-3 flex-shrink-0 group-hover:scale-110 transition-transform"/> : <FileText size={18} className="text-blue-500 mr-3 flex-shrink-0"/>}
                ...
                {isPreflight && manual.content.preflight && (
                    <div className="p-0 flex flex-col h-full animate-fade-in bg-[#F8FAFC]">
                        <div className="p-6 pb-2">
                             <div className="flex items-center gap-2 mb-2">
                                <Wrench className="text-[#F47A28]" size={20}/>
                                <h3 className="text-xl font-bold text-[#0C1A2A]">Pre-Flight Checklist</h3>
                             </div>
                             <p className="text-slate-500 text-sm mb-4">Confirm all equipment for <span className="font-bold text-slate-800">Option A: Well Water</span> is present.</p>
                        </div>
                                    ...
                                    <div className="space-y-3">
                                        {section.items.map((item, itemIdx) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className={`w-6 h-6 rounded border-2 flex-shrink-0 mt-0.5 flex items-center justify-center transition-colors duration-200 ${
                                                        isChecked 
                                                        ...
                                                        {itemUrl ? (
                                                            <div className="flex items-center gap-2">
                                                                <a 
                                                                    ...
                                                                    className={`font-bold text-sm leading-tight block flex items-center gap-1 hover:underline ${isChecked ? 'text-emerald-900' : 'text-blue-600 hover:text-blue-800'}`}
                                                                >
                                                                    {itemText} <ExternalLink size={10} />
                                                                </a>
                                                            </div>
                                                        ) : (
                                                             <span className={`font-medium text-sm leading-tight block ${isChecked ? 'text-emerald-900' : 'text-slate-700'}`}>
                                                                {itemText}
                                                            </span>
                                                        )}
                ...
                {isStep && (
                    <div className="p-6 md:p-8 flex flex-col h-full animate-fade-in overflow-y-auto">
                        <div className="flex items-center mb-4">
                             ...
                             <h3 className="text-lg md:text-xl font-bold text-[#0C1A2A] uppercase tracking-wider">Action Step</h3>
                        </div>
                        ...
                        <div className="flex-grow flex flex-col">
                                ...
                                <div className="mb-6 rounded-lg overflow-hidden border border-slate-200 shadow-sm max-h-60 relative group">
                                     <img 
                                        ...
                                        onError={(e) => {
                                          const target = e.target as HTMLImageElement;
                                          target.onerror = null; 
                                          target.src="https://via.placeholder.com/800x400?text=Image+Load+Error"
                                        }}
                                     />
                                     <button onClick={() => window.open(stepImage, '_blank')} className="absolute bottom-2 right-2 bg-black/50 p-1.5 rounded text-white hover:bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity">
                                         <Maximize2 size={16}/>
                                     </button>
                                </div>
                            )}
                            ...
                            <button 
                                onClick={() => onAskAI(`Can you explain step ${currentStepIndex + 1} ("${stepText}") in simple terms?`)}
                            ...
                            {(stepDiagram || stepCustomDiagram) && (
                                <div className="flex-grow min-h-[180px]">
                                    <BlueprintDiagram type={stepDiagram || 'custom'} customUrl={stepCustomDiagram} />
                                </div>
                            )}
                        ...
                        <div className="mt-auto pt-4 text-slate-400 text-sm flex items-center border-t border-slate-100">
                            <Play size={14} className="mr-2 text-[#F47A28]" fill="currentColor"/>
                ...
                {isCritical && (
                    <div className="p-8 flex flex-col h-full justify-center items-center text-center bg-red-50 animate-fade-in">
                        ...
                        <h2 className="text-2xl font-bold text-red-800 mb-2">CRITICAL WARNING</h2>
                        <p className="text-red-700 text-xl font-medium leading-relaxed max-w-lg">
                            {manual.content.critical}
                        </p>
                        <p className="mt-8 text-red-500 text-sm">Review this carefully. Failure here results in rework.</p>
                    </div>
                )}
                ...
                {isChecklist && (
                    <div className="p-6 flex flex-col h-full animate-fade-in overflow-y-auto">
                        <div className="flex justify-between items-center mb-6">
                             <h2 className="text-2xl font-bold text-[#0C1A2A]">Final Inspection</h2>
                        ...
                        <div className="space-y-3 flex-grow">
                             {manual.content.checklist.map((item, idx) => {
                                ...
                                return (
                                    <label key={idx} className={`flex items-start p-4 rounded-xl cursor-pointer transition-all duration-200 border-2 ${isChecked ? 'bg-emerald-50 border-emerald-500' : 'bg-white border-slate-100 hover:border-[#FFC947]'}`}>
                                    <div className="relative flex items-center mt-0.5">
                                        ...
                                        />
                                        <CheckCircle2 size={16} className="absolute left-0.5 top-0.5 text-white opacity-0 peer-checked:opacity-100 pointer-events-none" />
                                    </div>
                                    <span className={`ml-4 text-lg transition-colors ${isChecked ? 'text-emerald-900 font-medium' : 'text-slate-700'}`}>{item}</span>
                                    </label>
                                );
                            })}
                        </div>
                        ...
                        {checklistPercent === 100 && (
                         ...
                         {checklistPercent > 0 && checklistPercent < 100 && (
                            <div className="mt-4 flex justify-center">
                                 <button onClick={onReset} className="text-sm text-slate-400 hover:text-red-500 flex items-center gap-1">
...
const AdminEditor: React.FC<AdminEditorProps> = ({ manual, onSave, onCancel }) => {
    ...
    const [formData, setFormData] = useState<Manual>({
        ...
        lastUpdated: manual?.lastUpdated || new Date().toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }),
        content: {
    ...
    const handleChange = (field: keyof Manual, value: string) => {
        setFormData(prev => ({ ...prev, [field]: value }));
    };

    const handleContentChange = <K extends keyof ManualContent>(field: K, value: ManualContent[K]) => {
        setFormData(prev => ({ ...prev, content: { ...prev.content, [field]: value } }));
    };

    // Generic Array Handler
    const handleArrayChange = (field: 'checklist', index: number, value: string) => {
    ...
    const handleStepChange = (index: number, field: keyof ManualStepObj, value: string | null) => {
        const newSteps = [...formData.content.steps];
        const currentStep = newSteps[index];
        
        // Initialize object if currently string or create copy
        let stepObj: ManualStepObj = typeof currentStep === 'string' 
            ? { text: currentStep, diagram: null, image: null, exampleImage: null, customDiagram: null } 
            : { ...currentStep };
        
        if (field === 'text' && value !== null) stepObj.text = value;
        if (field === 'diagram') stepObj.diagram = value === 'none' ? null : value;
        if (field === 'image') stepObj.image = value === '' ? null : value; // Handle image URL
        if (field === 'exampleImage') stepObj.exampleImage = value === '' ? null : value; // Handle Example Image URL
        if (field === 'customDiagram') stepObj.customDiagram = value === '' ? null : value; // Handle Custom Diagram URL
        
        // Optimize: convert back to string if no extra props (optional, but cleaner data)
        if (!stepObj.diagram && !stepObj.image && !stepObj.exampleImage && !stepObj.customDiagram) {
    ...
    const addStep = () => handleContentChange("steps", [...formData.content.steps, ""]);
    const removeStep = (idx: number) => handleContentChange("steps", formData.content.steps.filter((_, i) => i !== idx));

    // --- Resource Handler ---
    const addResource = () => handleContentChange("resources", [...(formData.content.resources || []), { type: 'pdf', title: '', url: '' }]);
    const updateResource = (idx: number, field: keyof Resource, val: string) => {
        const newRes = [...(formData.content.resources || [])];
        newRes[idx] = { ...newRes[idx], [field]: val };
        handleContentChange("resources", newRes);
    };
    const removeResource = (idx: number) => handleContentChange("resources", (formData.content.resources || []).filter((_, i) => i !== idx));

    // --- Preflight Handler ---
    const ensurePreflightStruct = (): PreflightSection[] => {
        if (!formData.content.preflight || formData.content.preflight.length === 0) {
    ...
    const updatePreflightItem = (idx: number, val: string) => {
        const currentPre = ensurePreflightStruct();
    ...
    const removePreflightItem = (idx: number) => {
        const currentPre = ensurePreflightStruct();
    ...
    const [openImageInput, setOpenImageInput] = useState<number | null>(null);
        ...
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col h-[600px] animate-fade-in overflow-hidden">
            ...
            <div className="bg-slate-100 p-4 border-b border-slate-200 flex justify-between items-center">
                <div className="flex items-center">
                    {manual ? <Edit className="mr-2 text-slate-500" size={20}/> : <Plus className="mr-2 text-slate-500" size={20}/>}
                    <div>
                ...
                <button onClick={onCancel} className="text-slate-400 hover:text-red-500"><X size={20}/></button>
            </div>
            ...
            <div className="flex-grow p-6 overflow-y-auto">
                ...
                {editorStep === 0 && (
                    <div className="space-y-6 animate-fade-in">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Procedure Title</label>
                            <input 
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Overview & Scope</label>
                            <textarea 
                    ...
                    <div className="space-y-8 animate-fade-in">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex justify-between">
                                ...
                                <button onClick={addResource} className="text-blue-600 flex items-center gap-1 hover:underline"><Plus size={12}/> Add</button>
                            </label>
                            <div className="space-y-3">
                                {formData.content.resources?.map((res, idx) => (
                                    <div key={idx} className="flex gap-2 items-start bg-slate-50 p-2 rounded border border-slate-200">
                                        ...
                                        <button onClick={() => removeResource(idx)} className="text-slate-400 hover:text-red-500 p-1"><Trash2 size={16}/></button>
                                    </div>
                                ))}
                                {(!formData.content.resources || formData.content.resources.length === 0) && <p className="text-sm text-slate-400 italic">No resources attached.</p>}
                            </div>
                        </div>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex justify-between">
                                ...
                                <button onClick={addPreflightItem} className="text-blue-600 flex items-center gap-1 hover:underline"><Plus size={12}/> Add Item</button>
                            </label>
                            <div className="space-y-2">
                                {ensurePreflightStruct()[0].items.map((item, idx) => (
                                    <div key={idx} className="flex gap-2 items-center">
                                        ...
                                        />
                                        <button onClick={() => removePreflightItem(idx)} className="text-slate-300 hover:text-red-500"><X size={16}/></button>
                                    </div>
                                ))}
                                {ensurePreflightStruct()[0].items.length === 0 && <p className="text-sm text-slate-400 italic">No equipment listed.</p>}
                            </div>
                        </div>
                    </div>
                )}
                ...
                {editorStep === 2 && (
                    <div className="space-y-4 animate-fade-in">
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 flex justify-between items-center">
                            ...
                            <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded">Drag & Drop Disabled</span>
                        </label>
                        ...
                        {formData.content.steps.map((step, idx) => {
                            ...
                            return (
                                <div key={idx} className="bg-slate-50 p-3 rounded-lg border border-slate-200 group relative">
                                    <div className="flex gap-3 mb-2">
                                        <span className="bg-slate-200 w-6 h-6 rounded-full text-slate-600 font-bold text-xs flex items-center justify-center flex-shrink-0 mt-1">{idx + 1}</span>
                                        <textarea 
                                        ...
                                        />
                                        <button onClick={() => removeStep(idx)} className="text-slate-300 hover:text-red-500 self-start"><X size={16}/></button>
                                    </div>
                                    ...
                                    <div className="ml-9 flex items-center gap-3 flex-wrap">
                                        ...
                                        <div className="flex items-center gap-1">
                                            <Maximize2 size={12} className={diagVal ? "text-blue-500" : "text-slate-300"}/>
                                            <select 
                                                className={`text-xs p-1 rounded border ${diagVal ? "border-blue-300 bg-blue-50 text-blue-700 font-bold" : "border-slate-200 text-slate-500"}`}
                                                value={diagVal || 'none'}
                                                onChange={(e) => handleStepChange(idx, 'diagram', e.target.value)}
                                            >
                                        ...
                                        {/* Image Button */}
                                        <button 
                                            onClick={() => setOpenImageInput(openImageInput === idx ? null : idx)}
                                            className={`flex items-center gap-1 text-xs border rounded p-1 hover:bg-slate-100 ${imgVal || exampleImgVal ? 'border-purple-300 bg-purple-50 text-purple-700 font-bold' : 'border-slate-200 text-slate-500'}`}
                                        >
                                            <ImageIcon size={12} />
                                    ...
                                    {diagVal === 'custom' && (
                                        <div className="ml-9 mt-2 p-2 bg-blue-50 border border-blue-200 rounded flex gap-2 items-center">
                                            <span className="text-xs text-blue-700 font-bold w-20">Blueprint URL:</span>
                                            <input 
                                                ...
                                                onChange={(e) => handleStepChange(idx, 'customDiagram', e.target.value)}
                                            />
                                        </div>
                                    )}

                                    {/* Collapsible Image URL Inputs */}
                                    {(openImageInput === idx || imgVal || exampleImgVal) && (openImageInput === idx) && (
                                        <div className="ml-9 mt-2 p-2 bg-white border rounded space-y-2">
                                            <div className="flex gap-2 items-center">
                                                <span className="text-xs text-slate-400 w-20">Header Img:</span>
                                                <input 
                                            ...
                                            <div className="flex gap-2 items-center">
                                                <span className="text-xs text-slate-400 w-20">Example:</span>
                                                <input 
                                                    className="flex-1 text-xs border border-slate-200 rounded p-1 outline-none focus:border-purple-400"
                                                    value={exampleImgVal || ''}
                                                    placeholder="URL for example BELOW diagram..."
                                                    onChange={(e) => handleStepChange(idx, 'exampleImage', e.target.value)}
                                                />
                                            </div>
                                        </div>
                                    )}
                        ...
                        <button onClick={addStep} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-lg text-slate-500 font-bold hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-colors flex justify-center items-center">
                    ...
                    <div className="space-y-8 animate-fade-in">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex justify-between">
                                ...
                                <button onClick={() => handleContentChange("checklist", [...formData.content.checklist, ""])} className="text-blue-600 flex items-center gap-1 hover:underline"><Plus size={12}/> Add</button>
                            </label>
                            <div className="space-y-2">
                                {formData.content.checklist.map((item, idx) => (
                                    <div key={idx} className="flex gap-2 items-center">
                                        ...
                                        <input 
                                            ...
                                            value={item}
                                            onChange={(e) => handleArrayChange("checklist", idx, e.target.value)}
                                            placeholder="Verification item..."
                                        />
                                        <button onClick={() => handleContentChange("checklist", formData.content.checklist.filter((_, i) => i !== idx))} className="text-slate-300 hover:text-red-500"><X size={16}/></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                             <label className="block text-xs font-bold text-red-800 uppercase tracking-wider mb-2 flex items-center">
                                <AlertTriangle size={14} className="mr-1"/> Critical Failure Point
                            </label>
                            <textarea 
                                ...
                                onChange={(e) => handleContentChange("critical", e.target.value)}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-between">
                <button 
                    ...
                    className={`px-6 py-2 rounded-lg font-bold text-sm ${editorStep === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-600 hover:bg-white border border-transparent hover:border-slate-300'}`}
                >
                    Back
                </button>
...
const CategoryEditor: React.FC<CategoryEditorProps> = ({ onSave, onCancel }) => {
  ...
  const handleSave = () => {
    ...
    const selectedColor = colors.find(c => c.id === colorTheme)?.class || "bg-slate-100 text-slate-600";
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-6 animate-fade-in max-w-lg mx-auto">
       ...
       <div className="space-y-4">
                ...
                {Object.keys(ICON_MAP).map(iconKey => {
                    ...
                    return (
                        <button 
                            ...
                            className={`p-2 rounded border flex justify-center items-center ${iconName === iconKey ? 'bg-blue-50 border-blue-500 text-blue-600' : 'border-slate-200 text-slate-400 hover:bg-slate-50'}`}
                        >
                            <Icon size={20} />
                        </button>
                    )
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1">Color Theme</label>
            <div className="flex flex-wrap gap-2">
                {colors.map(c => (
                    <button
                        key={c.id}
                        onClick={() => setColorTheme(c.id)}
                        className={`w-8 h-8 rounded-full border-2 flex items-center justify-center ${c.class} ${colorTheme === c.id ? 'border-slate-900' : 'border-transparent'}`}
                    >
       ...
       <div className="flex justify-end gap-3 mt-6">
            ...
            <button onClick={handleSave} className="px-6 py-2 bg-[#0C1A2A] text-white rounded-lg font-bold hover:bg-[#1A2C42]">Create</button>
       </div>
    </div>
  );
};
...
const Footer: React.FC<FooterProps> = ({ onAdminClick, isAdmin }) => (
  <footer className="bg-[#0C1A2A] text-slate-400 py-6 text-center text-sm mt-auto">
    ...
    <div className="flex flex-col items-center gap-2">
      <p>&copy; {new Date().getFullYear()} LC Custom Construction. Internal Use Only.</p>
      <button 
        onClick={onAdminClick}
        className={`flex items-center gap-1 text-xs px-3 py-1 rounded-full transition-colors ${isAdmin ? "bg-[#FFC947] text-[#0C1A2A] font-bold" : "bg-slate-800 hover:bg-slate-700"}`}
      >
        <Lock size={10} />
...
const App: React.FC = () => {
  ...
  const handleChecklistToggle = (manualId: string, index: string | number) => {
  ...
  const handleResetChecklist = (manualId: string) => {
      if(!window.confirm("Clear all checklist progress for this manual?")) return;
      setUserProgress(prev => {
          const newState = { ...prev };
  ...
  const handleCreateCategory = (newCategory: Partial<Category>) => {
        ...
        setAppData(prev => ({
            ...
            [id]: {
                ...
                iconName: newCategory.iconName || 'FileText',
                color: newCategory.color || 'bg-slate-100 text-slate-600',
                manuals: []
            }
        }));
        setView('home');
  ...
  const handleUpdateManual = (updatedManual: Manual) => {
    if (!activeCategory) return;
    setAppData(prev => {
        const newData = { ...prev };
        const catKey = activeCategory;
        const manualExists = newData[catKey].manuals.find(m => m.id === updatedManual.id);
  ...
  const handleDeleteManual = (manualId: string) => {
    ...
    setAppData(prev => {
        const newData = { ...prev };
        newData[activeCategory].manuals = newData[activeCategory].manuals.filter(m => m.id !== manualId);
        return newData;
    });
  };

  const startEdit = (manual: Manual | null) => {
  ...
  const filteredManuals = useMemo(() => {
    ...
    const results: (Manual & { categoryTitle: string; categoryKey: string })[] = [];
    Object.keys(appData).forEach(catKey => {
      ...
      category.manuals.forEach(manual => {
        if (
          ...
          results.push({ ...manual, categoryTitle: category.title, categoryKey: catKey });
        }
      });
    });
    return results;
  ...
  const handleAdminPreview = (manual: Manual) => {
      setActiveManual(manual);
      setView('manual');
  };
  ...
  const resetData = () => {
      if(window.confirm("Factory Reset: This will erase all your changes and restore default manuals. Continue?")) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col relative">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 flex-grow w-full">
        ...
        {isAdmin && view === 'home' && (
             <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6 flex justify-between items-center animate-fade-in">
                <div>
                    ...
                    <p className="text-sm text-orange-700">Select a category to add or edit manuals.</p>
                </div>
                <button onClick={resetData} className="text-xs bg-white border border-orange-200 text-orange-600 px-3 py-2 rounded hover:bg-orange-100 flex items-center">
        ...
        {view === 'home' && (
          <div className="animate-fade-in">
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-slate-800">Trade Categories</h2>
              <p className="text-slate-500">Select an area to view standard procedures.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {isAdmin && (
                ...
                >
                    ...
                    <h3 className="font-bold text-slate-500 group-hover:text-slate-800">Add Category</h3>
                </button>
              )}
            </div>
          </div>
        ...
        {/* VIEW: CATEGORY LIST */}
        {view === 'category' && activeCategory && appData[activeCategory] && (
          <div className="animate-fade-in">
            ...
            <div className="flex items-center space-x-3 mb-6">
              <div className={`p-3 rounded-lg ${appData[activeCategory].color}`}>
                {React.createElement(ICON_MAP[appData[activeCategory].iconName] || FileText, { size: 24 })}
              </div>
              <div>
                <h2 className="text-2xl font-bold text-slate-900">{appData[activeCategory].title}</h2>
                <p className="text-slate-500 text-sm">
                    {isAdmin ? "Tap a manual to edit it, or use the eye to preview" : "Select a procedure below"}
                </p>
              </div>
            </div>
        ...
        {view === 'manual' && activeManual && (
          <ManualWizard 
            manual={activeManual} 
            onBack={() => searchTerm ? setView('search') : setView('category')}
            progress={userProgress[activeManual.id]}
            onToggleCheck={(index) => handleChecklistToggle(activeManual.id, index)}
          ...
          <div className="animate-fade-in">
              ...
              {filteredManuals.map(manual => (
                <div key={manual.id} className="relative">
                  ...
                  <ManualListItem 
                    ...
                    isAdmin={isAdmin}
                    onDelete={() => alert("Please go to the specific category to delete manuals.")}
                    onView={(m) => {
                         setActiveCategory(manual.categoryKey);
            ...
            {filteredManuals.length === 0 && (
              <div className="text-center py-12">
                ...
                <button onClick={goHome} className="text-orange-500 font-medium mt-2">Return Home</button>
              </div>
            )}
          </div>
        )}
      ...
      {(activeCategory || activeManual) && (
        <AIChatWidget 
            categoryId={activeCategory || (activeManual ? Object.keys(appData).find(key => appData[key].manuals.find(m => m.id === activeManual!.id)) || 'default' : 'default')}
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden">
                ...
                <form onSubmit={handleAuthSubmit} className="p-6">
                    <p className="text-slate-600 text-sm mb-4 text-center">Enter the site supervisor password to edit manuals.</p>
                    <input 
                        type="password" 
                        autoFocus
                        className={`w-full p-3 border rounded-lg outline-none transition-all mb-2 ${authError ? 'border-red-500 bg-red-50' : 'border-slate-300 focus:border-[#FFC947]'}`}
                    ...
                    <div className="flex gap-3">
                        <button 
                            type="button"
                            onClick={() => {setShowAuthModal(false); setPasswordInput(''); setAuthError(false);}} 
